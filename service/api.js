...
function allowed(role, resource) {
  if (resource === "billing") return role === "keiri";
  if (resource === "work") return role === "tanto" || role === "supervisor";
  if (resource === "decision") return role === "supervisor" || role === "boss";
  return false;
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/api/import-csv") {
    let body = ""; req.on("data", c => body += c); req.on("end", () => { try { const events = csvToEvents(body); state = processEvents([...state.events, ...events]); json(res, 200, { imported: events.length, ...state }); } catch (e) { json(res, 400, { error: String(e.message || e) }); } }); return;
  }
