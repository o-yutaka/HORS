...
export function normalizeEvent(event) {
  return {
    id: String(event.id ?? `EV-${Math.random().toString(36).slice(2, 8)}`),
...
export function simulate30Days(debts, weights = DEFAULT_WEIGHTS) {
  ...
  const scenarios = [
    ["A", "全放置", () => ranked],
    ["B", "Top 1処理", () => ranked.slice(1)],
    ["C", "Top 3処理", () => ranked.slice(3)],
    ["D", "ランダム処理", () => ranked.filter((_, i) => i % 3 !== 0)]
  ];
  return scenarios.map(([key, label, pick]) => {
    const remaining = pick();
    const explosions = remaining.filter((d) => d.pressure_total >= 70).length;
    return {
      key, label,
      unresolved_count_30d: remaining.length,
      explosion_events: explosions,
      impact_cost: remaining.reduce((s, d) => s + d.counterfactual_cost, 0),
      downstream_block_count: remaining.reduce((s, d) => s + d.downstream_block_count, 0)
    };
  });
}
...
export function csvToEvents(csv) {
  ...
  return lines.map((line, i) => {
    ...
    const obj = Object.fromEntries(headers.map((h, j) => [h, parts[j]?.trim() ?? ""]));
    return normalizeEvent({ ...obj, id: obj.id || `CSV-${i + 1}` });
  });
}
